...
When a two-state status follows whether another field is filled, use a derived status with `kind: "presence"`, `sourceField`, `nonEmpty`, and `empty` (for example borrower present → Lent out, borrower empty → On shelf). Never put this coupling in `customRequirements`.
