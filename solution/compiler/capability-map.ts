...
export function classifyCapabilities(ir: NormalizedProductIR): RouteDecision {
  ...
  unsupported.push(...ir.customRequirements.filter((requirement) => !isBuiltInRequirement(requirement) && !knownInteraction));
  ...
  if (knownInteraction) supported.push(knownInteraction, ...ir.customRequirements);
